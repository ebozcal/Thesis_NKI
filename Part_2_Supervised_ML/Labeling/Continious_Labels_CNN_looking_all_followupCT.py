...
for patient, frame in PI_groups:
    ...
    if frame.shape[0] > 1:
        ...
        for i in range(len(frame.index)):
            ...
            for v in range(len(frame.index)):
                if i+1+v < (len(frame.index)):
                    v2 = frame['vol_ai'][frame.index[i+1+v]] # follow up volume values
                    if  frame['vol_ai'][frame.index[i+1+v]] == np.nan:
                        v2 =  frame['vol_ai'][frame.index[i+1+v+1]]
                    p1 = calculate_change(v2, frame['vol_ai'][frame.index[i]])
                    #print("v1, v2, p1", (frame['vol_ai'][frame.index[i]], v2, p1))
                    vc = abs(frame['vol_ai'][frame.index[i+1+v]] - frame['vol_ai'][frame.index[i]])
                    if p1 < -0.2 and vc >24: # if any follow up volume values verify this condition
                        frame.iloc[i, 3] = p1
                        df_merged.iloc[frame.index[i], 3] = p1
                        break
                    elif  p1 > 0.2 and vc >24:# if any follow up volume values does not verify  above statement but 
                        if p1 >1:
                            frame.iloc[i, 3] = np.nan
                            df_merged.iloc[frame.index[i], 3] = np.nan
                        else:                        #verify this condition
                            frame.iloc[i, 3] = p1
                            df_merged.iloc[frame.index[i], 3] = p1
                        break
                    elif  -0.2 < p1 < 0.2 or vc <24:
                        if p1 >1:
                            frame.iloc[i, 3] = np.nan
                            df_merged.iloc[frame.index[i], 3] = np.nan
                        else:                        #verify this condition
                            frame.iloc[i, 3] = p1
                            df_merged.iloc[frame.index[i], 3] = p1
                    print( df_merged.iloc[frame.index[i], 3])
    #if  z<5:
     #   print(frame)
      #  print("----------------------------------------------")
...
print("label_countts:", df_merged["label"])
...
df_1000.to_csv("COntinious_CNN_710_paths_labels_m_full_20.csv")
