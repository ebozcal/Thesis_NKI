# Work of Stefano Trebeschi and Kevin Groot Lipman
...
def safe_paired_pval(y_true, y_pred):
    return stats.wilcoxon(y_pred[y_true == 1], y_pred[y_true < 1]).pvalue if y_true.std() > 0 else np.nan
...
def classification_analysis(y_true, y_pred, groups=None, repeats=10000, CI=95):
    # create one group per sample if no group provided
    g = np.arange(y_true.size) if groups is None else groups
    ...
    for random_state in tqdm.tqdm(range(repeats)):
        ...
        f1.append(safe_f1score(df_['y_true'].values, np.asarray([(x > 0.5) * 1 for x in df_['y_pred'].values])))
        tp, fp, tn, fn = perf_measure(df_['y_true'].values, np.asarray([(x > 0.5) * 1 for x in df_['y_pred'].values]))
...
def correlation_analysis(y_true, y_pred, groups=None, repeats=10000, CI=95):
    # create one group per sample if no group provided
    g = np.arange(y_true.size) if groups is None else groups
    # arrange in dataframe
    df = pd.DataFrame(np.array([y_true, y_pred, g]).T, columns=['y_true', 'y_pred', 'g'])
    df = df.astype({'y_true': 'float', 'y_pred': 'float'})
    # bootstrap
    slopes, intercepts, r_values, p_values, std_errs = ([] for i in range(5))
    ...
    for random_state in tqdm.tqdm(range(repeats)):
        ...
        if groups is not None:  # seep up
            ...
            df_ = df.sample(frac=1., replace=True, random_state=random_state)
        # testing
        slope, intercept, r_value, p_value, std_err = stats.linregress(list(df_['y_true'].values),
                                                                       list(df_['y_pred'].values))
        slopes.append(slope)
    ...
    d = {
        'slope': "{:1.2f}".format(np.nanpercentile(slopes, 50)) + ' (' +
                 "{:1.2f}".format(np.nanpercentile(slopes, 50 - (CI / 2))) + ' - ' +
                 "{:1.2f}".format(np.nanpercentile(slopes, 50 + (CI / 2))) + ')',
        'intercept': "{:1.2f}".format(np.nanpercentile(intercepts, 50)) + ' (' +
                     "{:1.2f}".format(np.nanpercentile(intercepts, 50 - (CI / 2))) + ' - ' +
                     "{:1.2f}".format(np.nanpercentile(intercepts, 50 + (CI / 2))) + '; ',
        'r': "{:1.2f}".format(np.nanpercentile(r_values, 50)) + '(' +
             "{:1.2f}".format(np.nanpercentile(r_values, 50 - (CI / 2))) + ' - ' +
             "{:1.2f}".format(np.nanpercentile(r_values, 50 + (CI / 2))) + ')',
        'p_value': "{:1.5f}".format(np.nanpercentile(p_values, 50)) + ' (' +
                   "{:1.5f}".format(np.nanpercentile(p_values, 50 - (CI / 2))) + ' - ' +
                   "{:1.5f}".format(np.nanpercentile(p_values, 50 + (CI / 2))) + ')',
        'std_err': "{:1.2f}".format(np.nanpercentile(std_errs, 50)) + ' (' +
                   "{:1.2f}".format(np.nanpercentile(std_errs, 50 - (CI / 2))) + ' - ' +
                   "{:1.2f}".format(np.nanpercentile(std_errs, 50 + (CI / 2))) + ')'
    }
    return d
...
def segment_analysis(paths_true: list, paths_pred: list, repeats=10000, CI=95):
    ...
    for i, (p_true, p_pred) in enumerate(zip(tqdm.tqdm(paths_true), paths_pred)):
        try:
            ...
            print('Error loading {}'.format(p_pred))
            continue
    ...
    d = {
        ...
        'Pred Volume': "{:1.2f}".format(np.nanpercentile(predvol, 50)) + ' (' +
                       ...
                       "{:1.2f}".format(np.nanpercentile(predvol, 50 + (CI / 2))) + ') p = ' +
                        "{:1.4f}".format(stats.mannwhitneyu(df['Label_vol'], df['Pred_vol'])[-1]) + ')',
...
def pet_analysis(paths_true: list, paths_pred: list, cutoffs: list):
    import SimpleITK as sitk
    import os
    df = pd.DataFrame(columns=['ID', 'SENS', 'DSC', 'AE_vol'])
    for i, (p_true, p_pred) in enumerate(zip(tqdm.tqdm(paths_true), paths_pred)):
        try:
            img_label = sitk.ReadImage(p_true)
            arr_label = sitk.GetArrayFromImage(img_label)
            arr_pred_1 = sitk.GetArrayFromImage(sitk.ReadImage(p_pred))
        except:
            print('Error loading {}'.format(p_pred))
            continue
        voxel_sixe = np.prod(img_label.GetSpacing()) / 1000  # voxel size in cm³ (spacing in mm³)
        df.at[i, 'Label_vol'.format(item)] = np.sum(arr_label.flatten()) * voxel_sixe

        for item in cutoffs:
            arr_pred = (arr_pred_1 > item) * 1
            TP = np.sum(arr_pred * arr_label)
            FP = np.count_nonzero(arr_pred * 1 - arr_label * 1 == 1)
            FN = np.count_nonzero(arr_label * 1 - arr_pred * 1 == 1)
            TP_vol, FP_vol, FN_vol = (item * voxel_sixe for item in [TP, FP, FN])
            ...
            df.at[i, 'ID'] = p_true.split(os.sep)[-1].split('.')[0]
            df.at[i, 'DSC_{}'.format(item)] = np.round(DSC, 3)
            df.at[i, 'SENS_{}'.format(item)] = np.round(SENS, 3)

            df.at[i, 'AE_vol_{}'.format(item)] = np.round(np.abs(((TP_vol + FP_vol) - (TP_vol + FN_vol))), 2)
            df.at[i, 'Pred_vol_{}'.format(item)] = np.sum(arr_pred.flatten()) * voxel_sixe
    return df


def kappa_analysis(df, repeats=10000, CI=95, method='fleiss'):
    # bootstrap
    kappa = []
    for random_state in tqdm.tqdm(range(repeats)):
        # sampling
        df_ = df.sample(frac=1., replace=True, random_state=random_state)
        k = aggregate_raters(df_)[0]

        # testing
        if method == 'fleiss':
            kappa.append(fleiss_kappa(k))
        else:
            kappa.append(fleiss_kappa(k, method='randolph'))

    d = {
        'kappa': "{:1.2f}".format(np.nanpercentile(kappa, 50)) + ' (' +
                 "{:1.2f}".format(np.nanpercentile(kappa, 50 - (CI / 2))) + ' - ' +
                 "{:1.2f}".format(np.nanpercentile(kappa, 50 + (CI / 2))) + ')',
    }
    return d